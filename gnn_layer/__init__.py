...
gnn_layer
=========
GNN representation-and-discovery layer for QRA (SCAFFOLD).
...
Capabilities (see docs/GNN_IMPLEMENTATION.md):
...
STATUS: scaffold. ``GnnLayerConfig`` is fully implemented; all other modules expose
typed signatures + docstrings with ``NotImplementedError`` bodies. Heavy imports
(torch, sklearn, sentence-transformers) are performed lazily inside functions so
this package imports cleanly without those models being loaded.

The top-level entry point is :func:`gnn_layer.runner.run_gnn_analysis`, invoked from
``analysis/runner.py`` only when ``config.gnn_layer.enabled`` is True (default False).
"""
