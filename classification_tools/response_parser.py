...
def parse_purer_results(
    ...
    """
    Write PURER consensus labels onto the corresponding therapist Segment objects.

    Structurally identical to parse_all_results() but writes to purer_* fields
    instead of VAMMR fields. No name_to_id map needed — PURER construct IDs
    (0–4) are integer-encoded in the consensus output, same as VAMMR.

    Parameters
    ----------
    results_all : dict
        Raw LLM merge results keyed by segment_id (therapist segments only).
    segments : list[Segment]
        All segments (participant + therapist). Only therapist segments with
        matching segment_ids are updated.
    """
    segment_lookup = {s.segment_id: s for s in segments}
    ...
    for seg_id, result in results_all.items():
        ...
        seg.purer_rater_ids = list(rater_ids)
        seg.purer_rater_votes = list(rater_votes)

        agreement_level = consensus.get('agreement_level')
        n_agree = consensus.get('n_agree', 0)
        n_raters = consensus.get('n_raters', len(rater_ids) or 1) or 1
        seg.purer_agreement_level = agreement_level
        seg.purer_agreement_fraction = (n_agree / n_raters) if n_raters else 0.0
        seg.purer_needs_review = bool(consensus.get('needs_review', False))
        seg.purer_run_consistency = n_agree

        primary_stage = consensus.get('primary_stage')
        consensus_vote = consensus.get('consensus_vote')

        if primary_stage is not None:
            seg.purer_primary = primary_stage
            seg.purer_secondary = consensus.get('secondary_stage')
            seg.purer_confidence_primary = consensus.get('primary_confidence', 0.0)
            seg.purer_confidence_secondary = consensus.get('secondary_confidence')
            seg.purer_justification = consensus.get('justification', '')
            counts['classified'] += 1
            continue
    ...
    print(f"PURER parsing: {counts['classified']}/{total} therapist segments classified")
    if counts['abstained']:
        print(f"  Abstained: {counts['abstained']}")
    if counts['split']:
        print(f"  Split vote: {counts['split']}")
    if counts['all_raters_failed']:
        print(f"  All raters failed: {counts['all_raters_failed']}")
    if counts['legacy_or_malformed']:
        print(f"  Malformed: {counts['legacy_or_malformed']}")
