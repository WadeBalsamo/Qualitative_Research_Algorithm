...
ABS = 'data/Meta'
CACHE = '/tmp/ctx_only_qwen.npz'


def ctx_of(r, bysess, k=6):
...
def score(name, emb_or_preds, ncls, df, folds, is_preds=False):
    preds = emb_or_preds if is_preds else B.run_linear_probe(
        df, emb_or_preds, folds, GnnLayerConfig(vaamr_n_classes=ncls, vaamr_class_balance=True))
    res = H.score_arm(name, preds, df, ABS, ncls,
                      meta={'embedding': 'qwen-concat', 'method': name, 'seed': 42},
                      write_ledger=False)
    l, h = res['llm_axis'], res['human_axis']
    print(f"  {name:22s}: LLM κ={l['cohen_kappa_205']:.3f} {[round(x,3) for x in l['ci95']]}"
          f"  HUM κ={h['cohen_kappa']:.3f} {[round(x,3) for x in h['ci95']]}", flush=True)
...
def main():
    ...
    raw['segment_id'] = raw['segment_id'].astype(str)
    raw['start_time_ms'] = raw['start_time_ms'].fillna(0).astype(int)
    bysess = {s: g.sort_values('start_time_ms') for s, g in raw.groupby('session_id')}

    part = raw[raw.speaker == 'participant']
    ctx_txt = {r.segment_id: (ctx_of(r, bysess) or "[no prior context]")[:4500]
               for r in part.itertuples() if r.segment_id in tgt}
    ids = [s for s in ctx_txt if s in tgt]

    cvec = {}
    if os.path.isfile(CACHE):
        d = np.load(CACHE, allow_pickle=True)
        cvec = {str(s): v for s, v in zip(d['segment_ids'], d['embeddings'])}
    ...
    for s in range(0, len(todo), 8):
        ch = todo[s:s+8]
        v = np.asarray(
            er.embed_texts_remote([ctx_txt[i][:5000] for i in ch], cfg, is_query=True, timeout=150, max_retries=5),
            dtype=np.float32,
        )
        for k, i in enumerate(ch):
            cvec[i] = v[k]
        np.savez_compressed(CACHE,
                            segment_ids=np.array(list(cvec)),
                            embeddings=np.stack([cvec[i] for i in cvec]))
        print(f"  {min(s+8, len(todo))}/{len(todo)}", flush=True)
        _t.sleep(0.4)
    ...
    score('Q_isolated_chk',  iso, 6, df, folds)
    score('Q_concat_ctx',    cat, 6, df, folds)

    seg_ids, lab6, _ = B._prepare_labeled(df, cat, GnnLayerConfig(vaamr_n_classes=6))
