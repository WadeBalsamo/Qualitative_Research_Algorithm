...
def score(name, preds, ncls, df):
    res = H.score_arm(name, preds, df, ABS, ncls,
                      meta={'embedding': 'qwen-ctx', 'method': name, 'seed': 42, 'branch': 'scaler-ctx'},
                      write_ledger=False)
    l, h = res['llm_axis'], res['human_axis']
    print(f"  {name}: LLM κ={l['cohen_kappa_205']:.3f} {[round(x,3) for x in l['ci95']]}"
...
def main():
    ...
    texts = {r.segment_id: (ctx_of(r, bysess)[:3500] + "\n>>> [TARGET]: " + str(r.text)[:4000])
    ...
    for s in range(0, len(todo), 8):
        ch = todo[s:s+8]
        v = np.asarray(
            er.embed_texts_remote([texts[i][:5000] for i in ch], cfg, is_query=True, timeout=150, max_retries=5),
            dtype=np.float32,
        )
    ...
    score('S1_ctx_probe', B.run_linear_probe(df, emb, folds, GnnLayerConfig(vaamr_n_classes=6, vaamr_class_balance=True)), 6, df)

    # two-stage No-code gate on CONTEXT features (stack the two winning levers)
    seg_ids, lab6, _ = B._prepare_labeled(df, emb, GnnLayerConfig(vaamr_n_classes=6))
    ...
    for f, tr, te in B._iter_folds(seg_ids, fo, fl):
        bclf = LogisticRegression(max_iter=3000, class_weight='balanced').fit(
            B._stack_l2(emb, tr), [1 if L[s] == 5 else 0 for s in tr])
    ...
    score('S1_ctx_twostage', preds, 6, df)
