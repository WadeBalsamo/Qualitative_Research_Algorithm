...
def main():
    ...
    seg_ids, proba = proba_for_C(best[0], df, emb, folds)
    oof = RD.ensemble_from_proba(seg_ids, proba, NC, 'softavg')
    ...
    _artifact = {
        '_note': ('Raw committed backing for the HEADLINE winner (per-rater ensemble ens_softavg, '
                  'tuned C). Regenerate with: python experiments/classification_scaler/_csweep.py '
    ...
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '_csweep_results.json')
