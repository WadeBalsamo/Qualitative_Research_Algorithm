...
def score(name, emb_or_preds, ncls, df, folds, is_preds=False):
    ...
    res = H.score_arm(name, preds, df, ABS, ncls,
                      meta={'embedding': 'minilm', 'method': name, 'seed': 42},
                      write_ledger=False)
    l, h = res['llm_axis'], res['human_axis']
    print(f"  {name:20s}: LLM κ={l['cohen_kappa_205']:.3f} {[round(x,3) for x in l['ci95']]}"
          f"  HUM κ={h['cohen_kappa']:.3f} {[round(x,3) for x in h['ci95']]} (n={h['n']})", flush=True)
...
def main():
    ...
    ctx_txt = {r.segment_id: (ctx_of(r, bysess) or "[no prior context]")[:4000] for r in part.itertuples()}
    ...
    score('M_isolated',    iso_emb, 6, df, folds)
    score('M_concat_ctx',  cat_emb, 6, df, folds)
    score('M_combined',    cmb_emb, 6, df, folds)

    seg_ids, lab6, _ = B._prepare_labeled(df, cat_emb, GnnLayerConfig(vaamr_n_classes=6))
    ...
    for f, tr, te in B._iter_folds(seg_ids, fo, fl):
        bclf = LogisticRegression(max_iter=3000, class_weight='balanced').fit(
            B._stack_l2(cat_emb, tr), [1 if L[s] == 5 else 0 for s in tr])
        tr5 = [s for s in tr if L[s] < 5]
        sclf = LogisticRegression(max_iter=3000, class_weight='balanced').fit(
            B._stack_l2(cat_emb, tr5), [L[s] for s in tr5])
        for s in te:
            x = B._stack_l2(cat_emb, [s])
            preds[s] = 5 if bclf.predict(x)[0] == 1 else int(sclf.predict(x)[0])
    score('M_concat_twostage', preds, 6, df, folds, is_preds=True)
