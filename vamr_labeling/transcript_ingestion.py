...
pipeline and produces coherent segments for VA-MR classification.

Adapted from:
  - semantic_segmenter.py's embedding-based boundary detection
    (_compute_similarity_curve, _find_candidate_boundaries)
  - classify_ctl_construct_text_similarity.py's clause-level processing
    (spacy_tokenizer with method='clause')

The diarization repo produces JSON files containing LabeledSentence objects
from speaker_aligner.py. Each sentence has text, speaker, start, end, and
words fields.
"""
...
class TranscriptSegmenter:
    """
    Groups diarized sentences into coherent segments for VA-MR classification.

    Uses embedding-based similarity curves and pause signals (adapted from
    semantic_segmenter.py) with length constraints tuned for therapeutic
    dialogue rather than general transcription.
    """
    ...
    def segment_session(
        ...
        and returns Segment objects suitable for VA-MR classification.
        ...
        for run in participant_runs + therapist_runs:
            ...
            # Similarity curve (windowed cosine, from _compute_similarity_curve)
            sim_curve = self._compute_similarity_curve(embeddings, window=3)

            # Pause signal between sentences
    ...
    # ------------------------------------------------------------------
    # Similarity and pause curves (from semantic_segmenter.py)
    # ------------------------------------------------------------------

    def _compute_similarity_curve(
        self, embeddings: np.ndarray, window: int = 3
    ) -> np.ndarray:
        """Direct adaptation of semantic_segmenter._compute_similarity_curve."""
        n = len(embeddings)
        if n <= 1:
            return np.array([])
        sims = np.zeros(n - 1)
    ...
    def _compute_pause_curve(self, sentences: List[Dict]) -> np.ndarray:
        """Adapted from semantic_segmenter._compute_pause_curve."""
        n = len(sentences)
        if n <= 1:
            return np.array([])
        pauses = np.zeros(n - 1)
    ...
    def _boundary_valid(
        ...
        """Enforce min word count per segment."""
        if existing:
    ...
    def _build_segments(
        ...
        for start, end in zip(starts, ends):
            ...
            # Split oversized segments at midpoint
        ...
        # Merge trailing undersized segments
    ...
    def _merge_undersized(self, segments: List[Segment]) -> List[Segment]:
        """Merge any segment under min_words with its predecessor."""
        if len(segments) <= 1:
            return segments
        merged = [segments[0]]
    ...
    def _separate_speaker_runs(
        ...
        """Group consecutive sentences by the same speaker into runs."""
    ...
    def _is_participant(speaker_label: str) -> bool:
        """Heuristic: the diarization pipeline labels the primary speaker."""
...
def load_diarized_session(session_path: str) -> Dict[str, Any]:
    ...
    # The diarization repo nests sentences under 'sentences' or 'sections'
