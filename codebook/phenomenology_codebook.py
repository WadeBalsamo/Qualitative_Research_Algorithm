...
from .codebook_schema import Codebook, CodeDefinition, slugify


def get_phenomenology_codebook() -> Codebook:
    """Return the meditation phenomenology codebook as a structured Codebook."""

    _raw_codes = _get_raw_codes()
    codes = []
    for domain, domain_codes in _raw_codes.items():
        for entry in domain_codes:
            codes.append(CodeDefinition(
                code_id=slugify(entry['category']),
                category=entry['category'],
                domain=domain,
                description=entry['description'],
                subcodes=entry['codes'],
                inclusive_criteria=entry['inclusive'],
                exclusive_criteria=entry['exclusive'],
            ))
...
def _get_raw_codes():
    """Return the raw codebook data organized by domain."""
    ...
    return {
        ...
        'Cognitive': [
            ...
            },
            {
                ...
                'exclusive': 'In cases of mental fog, attempt to differentiate absence or thought from a Change in Executive Functioning. Corresponding flattening of emotions or a lack of affect should be coded under Affective Flattening. Expected cycles of drowsiness or dullness should not be coded here.',
            },
            {
