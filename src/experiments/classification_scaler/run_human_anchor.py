...
cfg6 = GnnLayerConfig(); cfg6.vaamr_n_classes=6; cfg6.vaamr_class_balance=True; cfg6.seed=SEED
...
def _f(v): return 'nan' if v is None or (isinstance(v,float) and not np.isfinite(v)) else f"{v:.3f}"
...
A1N_LLM, A1N_LLM_LO = res_a1n['llm_axis']['cohen_kappa_205'], res_a1n['llm_axis']['ci95'][0]
...
def _fit_probe_w(X, y, w):
    ...
    clf.fit(X,y,sample_weight=w); return clf

def _proba_full(clf, X, n=6):
    P=clf.predict_proba(X); full=np.zeros((len(X),n))
    for j,c in enumerate(clf.classes_):
        if 0<=int(c)<n: full[:,int(c)]=P[:,j]
    return full
...
def run_mix(weight):
    ...
    for f,tr,te in B._iter_folds(seg_ids, fold_of, fold_list):
        ...
        clf=_fit_probe_w(_Xrows(tr), np.array(ylab), np.array(w,dtype=float))
        Pr=_proba_full(clf, _Xrows(te))
        for s,row in zip(te,Pr): preds[s]=int(np.argmax(row))
    return preds
...
for w in [1.0,2.0,3.0,5.0,8.0,12.0,20.0]:
    ...
    mix_rows.append(show(f'(a) mix  weight={w:g}', r)); mix_rows[-1]['w']=w
...
base_proba={}
for f,tr,te in B._iter_folds(seg_ids, fold_of, fold_list):
    clf=_fit_probe_w(_Xrows(tr), np.array([llm_lab[s] for s in tr]), np.ones(len(tr)))
    Pr=_proba_full(clf, _Xrows(te))
    for s,row in zip(te,Pr): base_proba[s]=row
...
def fit_bias(LP, Y, lam):
    ...
    n,C=LP.shape; oh=np.eye(C)[Y]
    def fg(b):
        S=_softmax(LP+b); loss=-(oh*np.log(S+EPS)).sum()/n + lam*(b@b)
        grad=(S-oh).sum(0)/n + 2*lam*b; return loss,grad
    r=minimize(fg, np.zeros(C), jac=True, method='L-BFGS-B'); return r.x

def fit_scalebias(LP, Y, lam):
    """scalar temperature s>0 + per-class bias b (Guo vector-scaling, shared s)."""
    n,C=LP.shape; oh=np.eye(C)[Y]
    def fg(th):
        s=th[0]; b=th[1:]; Z=s*LP+b; S=_softmax(Z)
        loss=-(oh*np.log(S+EPS)).sum()/n + lam*(b@b)
        gb=(S-oh).sum(0)/n + 2*lam*b
        gs=((S-oh)*LP).sum()/n
        return loss, np.concatenate([[gs],gb])
    r=minimize(fg, np.concatenate([[1.0],np.zeros(C)]), jac=True, method='L-BFGS-B'); return r.x

def fit_stack(LP, Y, C_reg):
    clf=LogisticRegression(max_iter=5000, C=C_reg, class_weight='balanced', multi_class='multinomial')
    clf.fit(LP, Y); return clf

def run_recal(kind, hp):
    """LOPO over human participants; non-human participants use ALL human codes."""
    h_ids=list(human_cls); H_LP=np.stack([base_logp[s] for s in h_ids]); H_Y=np.array([human_cls[s] for s in h_ids])
    h_part=np.array([part_of[s] for s in h_ids])
    preds={}
    # cache a "global" fit (all human) for non-human participants
    def fit(mask):
        LP,Y=H_LP[mask],H_Y[mask]
        if kind=='bias': return ('p', fit_bias(LP,Y,hp))
        if kind=='scalebias': return ('s', fit_scalebias(LP,Y,hp))
        if kind=='stack': return ('c', fit_stack(LP,Y,hp))
    def apply(model, LP):
        t,m=model
        ...
        if t=='c':
            ...
            for j,c in enumerate(m.classes_): full[:,int(c)]=m.predict_proba(LP)[:,j]
...
for kind,sweep in [('bias',[0.0,0.05,0.2,1.0]), ('scalebias',[0.0,0.05,0.2,1.0]), ('stack',[0.1,0.5,2.0])]:
    for hp_ in sweep:
        ...
        row=show(f'(b) recal {kind}  reg={hp_:g}', r); row['kind']=kind; row['reg']=hp_
...
# ============================================================================
def run_human_only():
    h_ids=list(human_cls); h_part=np.array([part_of[s] for s in h_ids])
    HY=np.array([human_cls[s] for s in h_ids])
    preds={}
    # fit-all for non-human participants (LLM-axis 205 coverage)
    clf_all=_fit_probe_w(_Xrows(h_ids), HY, np.ones(len(h_ids)))
    per_part={}
    for p in human_parts:
        mask=h_part!=p
        per_part[p]=_fit_probe_w(_Xrows([h_ids[i] for i in range(len(h_ids)) if mask[i]]), HY[mask], np.ones(int(mask.sum())))
    for s in seg_ids:
        p=part_of[s]; clf=per_part[p] if p in per_part else clf_all
        preds[s]=int(np.argmax(_proba_full(clf,_Xrows([s]))[0]))
    return preds
...
# SUMMARY TABLE
# ============================================================================
print("\n\n================== SUMMARY (vs A1n: LLMκ=%.3f[lo %.3f], HUMκ=%.3f) =================="%(A1N_LLM,A1N_LLM_LO,res_a1n['human_axis']['cohen_kappa']))
...
line('A1n', A1N_LLM, res_a1n['llm_axis']['ci95'][0], res_a1n['llm_axis']['ci95'][1], A1N_HUM, res_a1n['human_axis']['ci95'][0], res_a1n['human_axis']['ci95'][1])
for r in mix_rows:   line(f"(a)mix w={r['w']:g}", r['llm'],r['llm_lo'],r['llm_hi'], r['hum'],r['hum_lo'],r['hum_hi'])
for r in recal_rows: line(f"(b){r['kind']} reg={r['reg']:g}", r['llm'],r['llm_lo'],r['llm_hi'], r['hum'],r['hum_lo'],r['hum_hi'])
line('(c)human-only', res_c['llm_axis']['cohen_kappa_205'],res_c['llm_axis']['ci95'][0],res_c['llm_axis']['ci95'][1], res_c['human_axis']['cohen_kappa'],res_c['human_axis']['ci95'][0],res_c['human_axis']['ci95'][1])
